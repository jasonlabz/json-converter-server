...
import (
	...
	_ "github.com/jasonlabz/json-converter-server/docs"
	"github.com/jasonlabz/json-converter-server/server/controller"
	diy "github.com/jasonlabz/json-converter-server/server/middleware"
)
...
func InitApiRouter() *gin.Engine {
	...
	rootMiddleware(router, diy.HandleStaticFile)
...
func registerV1GroupAPI(router *gin.RouterGroup) {
	router.POST("/convert", controller.HandleConvert)
	router.GET("/languages", controller.HandleGetLanguages)
	router.GET("/tags", controller.HandleGetTags)

	// 演示不同MIME类型的返回
	router.GET("/demo/image", controller.HandleDemoImage)
	router.GET("/demo/json", controller.HandleDemoJSON)
	router.GET("/demo/text", controller.HandleDemoText)
}
